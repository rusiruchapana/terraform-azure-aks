...
This capstone demonstrates a complete platform engineering workflow: infrastructure provisioning, application CI, GitOps delivery, environment promotion, observability, AIOps visibility, load testing, and cleanup.

```mermaid
flowchart TB
    learner["Engineer / Learner"]

    subgraph platform["Platform Repository: terraform-azure-aks"]
        terraform["Terraform Infrastructure"]
        platformCI["Terraform Platform CI"]
        aksPlatform["AKS Platform Foundation"]
        platformDocs["Capstone Guides"]
    end

    subgraph app["Application Repository: aks-capstone-store-app"]
        appCode["Store App Source Code"]
        appCI["Build and Scan Pipeline"]
        image["Container Image"]
        devVerify["Dev Release Verification"]
    end

    subgraph gitops["GitOps Repository: aks-capstone-gitops"]
        devOverlay["Dev Overlay"]
        qaOverlay["QA Overlay"]
        prodOverlay["Prod Overlay"]
        validation["GitOps Validation"]
        promotion["QA / Prod Promotion"]
        aiopsManifests["AIOps Manifests"]
    end
    ...
    subgraph argocd["Argo CD"]
        ...
        qaApp["QA App"]
        prodApp["Prod App"]
        aiopsApps["AIOps Apps"]
    end

    subgraph workloads["AKS Workloads"]
        dev["capstone-dev"]
        qa["capstone-qa"]
        prod["capstone-prod"]
        aiops["capstone-aiops"]
        monitoring["monitoring"]
    end

    subgraph operations["Operations and Proof"]
        gateway["Gateway HTTP Check"]
        grafana["Grafana Dashboards"]
        prometheus["Prometheus Metrics"]
        alertmanager["Alertmanager Alerts"]
        aiopsDashboard["AIOps Dashboard"]
        k6["k6 Load Test"]
        cleanup["Cost and Cleanup Guide"]
    end
    ...
    terraform --> platformCI
    platformCI --> aksPlatform
    aksPlatform --> aks
    ...
    devOverlay --> validation
    qaOverlay --> validation
    prodOverlay --> validation
    promotion --> qaOverlay
    promotion --> prodOverlay
    aiopsManifests --> validation

    validation --> devApp
    validation --> qaApp
    validation --> prodApp
    validation --> aiopsApps

    devApp --> dev
    qaApp --> qa
    prodApp --> prod
    aiopsApps --> aiops
    aiopsApps --> monitoring
...
1. Terraform creates the Azure and AKS platform foundation.
2. Platform CI checks Terraform formatting, validation, linting, and security.
3. The app repository builds, scans, and pushes the store-front image to ACR.
4. The Dev GitOps overlay is updated with the new image tag.
5. GitOps validation checks YAML, Kustomize rendering, and Kubernetes schema correctness.
6. Argo CD syncs Dev, QA, Prod, monitoring, and AIOps workloads to AKS.
7. Dev verification confirms ACR image, GitOps desired state, Argo CD health, AKS rollout, and Gateway response.
8. QA and Prod use a build-once, promote-same-image workflow.
9. Prometheus, Grafana, Alertmanager, and the AIOps Dashboard provide operational visibility.
10. k6 load testing proves the app remains healthy under controlled traffic.
11. The cost and cleanup guide explains how to safely pause or remove the environment.
